        ...
        try {
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    ...
                    // Crucial for a trading engine:
                    // - Update internal order state to 'SENT_TO_KAFKA'
                    // - Trigger downstream processing for successful order
                } else {
                    logger.error("Asynchronous Send (FAILURE): Failed to send order {}: {}",
                            order.getOrderId().toString(), ex.getMessage(), ex);
                    // CRITICAL for a trading engine:
                    // - Log the error prominently
                    // - Implement retry logic (e.g., using Spring Retry, or a custom mechanism)
                    // - Alert operations immediately (e.g., via monitoring system)
                    // - Mark the order as 'SEND_FAILED' or 'PENDING_RETRY' in your internal system
                    // - Consider compensatory actions if the order was already processed internally.
                }
            });
