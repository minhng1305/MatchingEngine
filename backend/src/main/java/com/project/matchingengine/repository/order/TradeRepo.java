...
public interface TradeRepo extends JpaRepository<Trade, UUID> {
    ...
    @Query("SELECT t FROM Trade t WHERE t.symbol = :symbol ORDER BY t.tradeTimestamp DESC LIMIT 1")
    Trade findLatestTradeBySymbol(@Param("symbol") String symbol);
    ...
    List<Trade> findByBuyerUserIdOrSellerUserIdOrderByTradeTimestampDesc(UUID buyerUserId, UUID sellerUserId);
}